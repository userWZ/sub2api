          ...
          <div class="flex items-center justify-between gap-3">
            <div>
              <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
                {{ t('keys.quotaDisabled') }}
              </span>
              <p class="mt-0.5 text-xs leading-5 text-gray-500 dark:text-gray-400">
                {{ t('keys.quotaDisabledHint') }}
              </p>
            </div>
            <button
              type="button"
              @click="formData.quota_disabled = !formData.quota_disabled"
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.quota_disabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
              ]"
            >
              <span
                :class="[
                  'pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                  formData.quota_disabled ? 'translate-x-4' : 'translate-x-0'
                ]"
              />
            </button>
          </div>
