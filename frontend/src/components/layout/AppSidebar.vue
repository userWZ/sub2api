...
//
// 条目顺序：密钥 → 用量 → 可用服务 → 渠道状态 → 订阅/支付 → 兑换/资料。
// 可用服务紧挨渠道状态之上，让用户"先看自己能用什么、再看对应状态"。
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
	...
	{ path: '/image-workbench', label: t('nav.imageWorkbench'), icon: ImageIcon, hideInSimpleMode: true },
...
// Admins access 可用服务 from this section just like regular users — there is no
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/managed-users', label: t('nav.managedKeys'), icon: KeyIcon, hideInSimpleMode: true },
    ...
    { path: '/admin/promo-codes', label: t('nav.promoCodes'), icon: GiftIcon, hideInSimpleMode: true },
    {
      ...
      children: [
		{ path: '/admin/affiliates/settings', label: t('nav.affiliateSettings'), icon: ShieldIcon },
		{ path: '/admin/affiliates/usage-rewards', label: t('nav.affiliateUsageRewards'), icon: GiftIcon },
    ...
    },
    {
      ...
      children: [
		...
		{ path: '/admin/orders/renewal', label: t('nav.renewalCampaign'), icon: GiftIcon },
      ],
    },
