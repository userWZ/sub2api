export default {
    ...
    renewal: {
      title: '订阅续订活动',
      description: '独立配置续订资格、折扣、未用额度结转、邮件提醒及邮件模板。',
      orderNotice: '配置只在用户创建新的订阅订单时计算并固化；不会追溯修改历史订单，也不会触发任何系统退款。结转额度在支付成功并完成续订后进入用户余额积分。',
      baseTitle: '活动与续订资格',
      baseHint: '总开关控制整项活动；关闭后可继续预设各子模块参数，但不会向用户生效。',
      running: '活动运行中',
      stopped: '活动已停止',
      offerEnabled: '启用续订活动',
      offerEnabledHint: '只有开启后，符合时间窗口的续订订单才会计算折扣或结转权益，续订活动邮件也才会发送。',
      beforeDays: '到期前可续订天数',
      beforeDaysHint: '从到期前多少天开始允许获得续订权益，0 表示仅从到期时开始。',
      afterDays: '到期后可续订天数',
      afterDaysHint: '订阅到期后仍可获得续订权益的天数，0 表示到期后不再享受。',
      daysUnit: '天',
      eligibilityPreview: '资格窗口：订阅到期前 {before} 天至到期后 {after} 天；系统按用户和同一订阅组识别续订。',
      discountTitle: '续订折扣',
      discountHint: '控制新续订订单的实付金额，折扣金额在下单时写入订单快照。',
      discountEnabled: '启用续订折扣',
      discountEnabledHint: '可以单独关闭折扣，同时保留额度结转或邮件提醒。',
      discountPercent: '折扣优惠比例',
      discountMinOrder: '享受折扣的最低订单金额',
      discountMaxAmount: '单笔最高优惠金额',
      discountPreview: '示例：原价 {amount}，续订优惠 {discount}，折后金额 {payable}。实际结果还会受到最低订单金额和封顶金额控制。',
      rolloverTitle: '未用额度结转',
      rolloverHint: '按当前订阅组的月额度减去本月已使用额度计算剩余值，再将配置比例转为余额积分。',
      rolloverEnabled: '启用未用额度结转',
      rolloverEnabledHint: '可以单独关闭结转，同时保留续订折扣或邮件提醒。结转在支付成功后幂等发放。',
      rolloverPercent: '剩余额度结转比例',
      rolloverMinUnused: '触发结转的最低未用额度',
      rolloverMaxAmount: '单笔最高结转积分',
      rolloverPreview: '示例：未用月额度 {unused}，预计结转 {rollover} 积分；实际结果还会受到最低未用额度和封顶值控制。',
      zeroNoLimit: '填写 0 表示不设置门槛或上限。',
      emailTitle: '续订邮件提醒',
      emailHint: '邮件发送开关、提醒节点和模板均在本页管理；每个订阅在每个提醒节点最多发送一次。',
      emailEnabled: '启用续订活动邮件',
      emailEnabledHint: '邮件开关独立于折扣和结转开关，但仍受续订活动总开关控制。',
      reminderDays: '到期前提醒节点',
      reminderDaysHint: '可以选择多个节点，也可以添加自定义天数；节点必须位于到期前资格窗口内。',
      expiryDay: '到期当天',
      daysBefore: '提前 {day} 天',
      customReminderPlaceholder: '自定义提前天数',
      reminderRequired: '启用邮件时至少需要保留一个提醒节点。',
      invalidReminderDay: '提醒天数必须是 0 到 {max} 之间的整数。',
      invalidForm: '请检查续订活动配置中的范围、比例和提醒节点。',
      saved: '续订活动设置已保存',
      movedHint: '续订活动已经从系统设置拆分，可在独立页面配置各模块开关、门槛、封顶和邮件模板。',
      openSettings: '打开续订活动设置',
      errors: { loadFailed: '加载续订活动设置失败' }
    },
    ...
    affiliates: {
	  ...
	  settings: {
		title: '邀请返利活动设置',
		description: '集中配置充值返利、返利钱包、首次用量奖励和邀请风控。',
		creditNotice: '首次用量奖励发放的是系统内计费额度积分，将直接进入邀请人的余额；不代表现金，不触发系统退款。',
		baseTitle: '邀请返利基础规则',
		baseHint: '控制邀请关系、充值返利及返利钱包的使用方式。',
		enabled: '启用邀请返利',
		enabledHint: '关闭后不再绑定新的邀请关系，也不会产生新的返利或首次用量奖励。',
		rebateRate: '充值返利比例',
		freezeHours: '返利冻结时间（小时）',
		durationDays: '邀请返利有效期（天）',
		perInviteeCap: '单个被邀请人累计返利上限',
		discountEnabled: '允许返利钱包抵扣订单',
		discountEnabledHint: '用户下单时可以使用可用返利额度抵扣部分订单金额。',
		discountMaxPercent: '单笔最高抵扣比例',
		discountMinPay: '单笔最低实付额度',
		usageRewardTitle: '首次用量邀请奖励',
		usageRewardHint: '被邀请人第一次产生大于 0 的实际计费用量时，为邀请人生成一次奖励。每个被邀请人最多触发一次。',
		usageRewardEnabled: '启用首次用量奖励',
		usageRewardEnabledHint: '建议完成风控阈值配置后再开启。默认关闭。',
		rewardAmount: '奖励积分',
		rewardAmountHint: '默认 20，直接计入邀请人的系统计费余额。',
		riskTitle: '奖励风控指标',
		riskHint: '命中任一规则的奖励进入待审核，不会自动增加余额。',
		openReview: '打开奖励审核',
		inviterDailyLimit: '同一邀请人 24 小时奖励上限',
		inviter30dLimit: '同一邀请人滚动 30 天奖励上限',
		ipDailyLimit: '同一 IP 24 小时触发上限',
		zeroUnlimited: '填写 0 表示不使用这条数量限制。',
		reviewSameIP: '邀请双方同 IP 时转人工审核',
		reviewSameIPHint: '将触发 IP 与邀请人最近一次成功用量 IP 比较。',
		reviewMissingIP: '缺少触发 IP 时转人工审核',
		reviewMissingIPHint: '批量任务或代理链无法提供可靠 IP 时不会自动发放。',
		saved: '邀请返利设置已保存'
	  },
	  rewards: {
		title: '首次用量奖励审核',
		description: '查看自动发放与命中风控的邀请奖励，并处理待审核记录。',
		searchPlaceholder: '搜索邀请人或被邀请人的邮箱、用户名',
		allStatuses: '全部状态',
		empty: '暂无首次用量奖励记录',
		review: '审核',
		reviewTitle: '审核邀请奖励',
		reviewSummary: '奖励 {amount} 积分；风控原因：{reason}',
		reviewRemark: '审核备注',
		reviewRemarkPlaceholder: '填写核验依据或处理说明（选填）',
		approve: '通过并发放',
		reject: '拒绝奖励',
		approved: '奖励已通过并发放',
		rejected: '奖励已拒绝',
		columns: { inviter: '邀请人', invitee: '被邀请人', amount: '奖励积分', status: '状态', riskReason: '风控原因', triggerIP: '触发 IP', createdAt: '触发时间', actions: '操作' },
		status: { pending: '待审核', granted: '已发放', rejected: '已拒绝' },
		reasons: { auto_granted: '自动通过', missing_ip: '缺少触发 IP', same_ip: '邀请双方同 IP', inviter_daily_limit: '邀请人 24 小时超限', inviter_30d_limit: '邀请人 30 天超限', ip_daily_limit: '同一 IP 24 小时超限', manual_approved: '人工通过', manual_rejected: '人工拒绝' }
	  },
      errors: {
