    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          enabled: '启用邀请返利',
          enabledHint: '关闭后用户菜单中的邀请页面入口隐藏、注册时忽略邀请码、新付款不再产生返利。已有人民币返利仍可用于抵扣或由管理员提现。',
          rebateRate: '全局返利比例',
          rebateRateHint: '按被邀请人实际付款金额返给邀请人的默认比例（0-100%，例如填写 10 表示返利 10%）。',
      ...
      payment: {
        ...
        balanceRechargeMultiplierHint: '用户每支付 1 CNY 可获得多少积分',
        balanceRechargePreview: '预览：1 CNY = {points} 积分',
		renewalOffer: '订阅续订激励',
		renewalOfferHint: '在订阅到期前后指定天数内，自动应用续订折扣并将部分未用月额度结转到余额。',
		renewalWindowDays: '续订窗口（到期前后天数）',
		renewalDiscountPercent: '续订优惠（%）',
		renewalRolloverPercent: '未用月额度结转（%）',
		renewalEmail: '发送续订权益提醒邮件',
