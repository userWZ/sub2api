...
type APIKey struct {
	...
	// Disable API-key-level quota accounting; user balance/subscription billing still applies
	...
	for i := range columns {
		...
		case apikey.FieldQuotaDisabled:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field quota_disabled", values[i])
			} else if value.Valid {
				_m.QuotaDisabled = value.Bool
			}
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("quota_disabled=")
	builder.WriteString(fmt.Sprintf("%v", _m.QuotaDisabled))
	builder.WriteString(", ")
