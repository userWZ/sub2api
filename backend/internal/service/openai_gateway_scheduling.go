...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusions(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability) (*Account, error) {
	...
	if account := s.tryStickySessionHit(ctx, groupID, platform, sessionHash, requestedModel, excludedIDs, requireCompact, stickyAccountID, requiredCapability, accountAllowlist); account != nil {
		return account, nil
	}
	...
	selected, compactBlocked := s.selectBestAccount(ctx, groupID, platform, accounts, requestedModel, excludedIDs, requireCompact, requiredCapability, accountAllowlist)
...
func (s *OpenAIGatewayService) tryStickySessionHit(ctx context.Context, groupID *int64, platform string, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability, accountAllowlist map[int64]struct{}) *Account {
	if sessionHash == "" {
		return nil
	}
	platform = normalizeOpenAICompatiblePlatform(platform)
	...
	if !imageWorkbenchAccountAllowed(accountAllowlist, accountID) {
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, platform string, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, accountAllowlist map[int64]struct{}) (*Account, bool) {
	platform = normalizeOpenAICompatiblePlatform(platform)
	...
	for i := range accounts {
		...
		if !imageWorkbenchAccountAllowed(accountAllowlist, acc.ID) {
			continue
		}
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability) (*AccountSelectionResult, error) {
	...
	if sessionHash != "" {
		accountID := stickyAccountID
		if accountID > 0 && !isExcluded(accountID) && imageWorkbenchAccountAllowed(accountAllowlist, accountID) {
			account, err := s.getSchedulableAccount(ctx, accountID)
			if err == nil {
	...
	tryAcquireFromLoadMap := func(loadMap map[int64]*AccountLoadInfo) (*AccountSelectionResult, bool, error) {
		...
		for _, item := range selectionOrder {
			...
			if !imageWorkbenchAccountAllowed(accountAllowlist, fresh.ID) {
				continue
			}
