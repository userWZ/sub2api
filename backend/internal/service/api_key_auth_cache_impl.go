...
const apiKeyAuthSnapshotVersion = 17 // v17: combine OceanWay snapshot fields with group web search per-call pricing
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		QuotaDisabled: apiKey.QuotaDisabled,
		Quota:         apiKey.Quota,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		QuotaDisabled: snapshot.QuotaDisabled,
