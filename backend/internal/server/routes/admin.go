...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		// 生图工作台固定账号池配置
		adminSettings.GET("/image-workbench-accounts", h.Admin.Setting.GetImageWorkbenchAccountSettings)
		adminSettings.PUT("/image-workbench-accounts", h.Admin.Setting.UpdateImageWorkbenchAccountSettings)
		// 429默认回避配置
...
func registerAffiliateRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	{
		affiliates.GET("/settings", h.Admin.Affiliate.GetSettings)
		affiliates.PUT("/settings", h.Admin.Affiliate.UpdateSettings)
		...
		affiliates.GET("/usage-rewards", h.Admin.Affiliate.ListUsageRewards)
		affiliates.POST("/usage-rewards/:reward_id/review", h.Admin.Affiliate.ReviewUsageReward)
		...
		users := affiliates.Group("/users")
		{
			...
			users.POST("/:user_id/withdraw", h.Admin.Affiliate.WithdrawAffiliateQuota)
